...
import { deleteTech } from '@/actions/admin/deleteTech';
...
const TableTech = ({ techStack }: Props) => {
  ...
  const handleDelete = async (techId: string) => {
    ...
    if (ok) {
      ...
      try {
        const result = await deleteTech(techId);

        if (result.success) {
          toast({
            title: 'Berhasil!',
            description: 'Tech berhasil dihapus.',
            variant: 'success',
          });

          setRows((prevRows) => prevRows.filter((project) => project.id !== techId));
        } else {
          toast({
            title: 'Gagal!',
            description: result.message || 'Terjadi kesalahan saat menghapus tech.',
            variant: 'destructive',
          });
        }
      } catch (error) {
          ...
          <table className="w-full divide-y divide-foreground">
            <thead className="bg-background">
              <tr>
                ...
                <th className="px-6 py-4 text-left text-xs font-medium text-muted-foreground  uppercase tracking-wider">Image</th>
                <th className="px-6 py-4 text-left text-xs font-medium text-muted-foreground  uppercase tracking-wider">Time</th>
                <th className="px-6 py-4 text-left text-xs font-medium text-muted-foreground  uppercase tracking-wider">Updated At</th>
                ...
                <tr key={row.id} className="hover:bg-gray-200 dark:hover:bg-gray-950 transition-colors">
                  ...
                  <td className="px-6 py-4 whitespace-nowrap">
                    <div className="flex items-center gap-4">
                      <CustomDialog textButton="Update" titleDialog="Form">
                        <TechStackForm techStack={row} />
                      </CustomDialog>
